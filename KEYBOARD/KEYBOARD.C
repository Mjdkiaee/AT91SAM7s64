...
int keyBoard()
{
	unsigned int * PIO_Enable = (unsigned int*) 0xfffff400;  //  PIO Enable Register
	unsigned int * PIO_OER = (unsigned int * ) 0xfffff410 ;   // Output Enable Register
	unsigned int *PIO_ODR = (unsigned int * ) 0xfffff414 ;   // Output Disable Register
	
	unsigned int * PIO_SODR = (unsigned int * ) 0xfffff430 ;   // Set Output Data Register
	unsigned int *PIO_CODR = (unsigned int * ) 0xfffff434 ;   //Clear Output Data Register
	unsigned int *PIO_ODSR = (unsigned int * ) 0xfffff438 ;   //Output Data Status Register
	unsigned int *PIO_PDSR = (unsigned int * ) 0xfffff43C ;   //Pin Data Status Register
	...
	unsigned int  RowR = 0;
	unsigned int ColumnR= 0; 
		
	unsigned int RowW= 0;
	unsigned int Columnw= 0;
	unsigned int  maskArray[] ={1<<5 , 1<<28 , 1<<30, 1<<2 };
	
	
	* PIO_ODR=	(1<<11) |  (1<<12)   |  1<<13   |  1<<14 ; // row output enable , row is input
	* PIO_SODR= (1<<11) |  (1<<12)   |  1<<13   |  1<<14 ;    // pull up rows;
	* PIO_OER= 1<<5 | 1<<28 | 1<<30 | 1<<2;	   // column is output
	
	
	 RowR = ((*PIO_PDSR) >>11)&0xF; // row = pin 11,12,13,14
	...
	for(rowNumber=0;rowNumber<4;rowNumber++)
	{
		...
		* PIO_SODR = (maskArray[rowNumber] & (0xffffffff))  ;  // use maskarray to arreng output
		
		RowR = ((*PIO_PDSR) >>11)&0xF;
	...
	//i//f((RowR & maskArray[i])==0)
	
	
	
	
	 *PIO_CODR= 1<<5 | 1<<28 | 1<<30 | 1<<2;
   RowR = (((*PIO_PDSR) >>11) & (0xF));
	for(;;)
	{
    	while(RowR !=0x0f)
